...
def get_pascal_panoptic_files(image_dir, gt_dir, json_info):
    files = []
    ...
    for ann in json_info["annotations"]:
        ...
        label_file = os.path.join(gt_dir, ann["file_name"].replace('.jpg', '.png'))
...
def load_pascal_panoptic_parts(image_dir, gt_dir, gt_json, meta):
    """
    Args:
    ...
    assert os.path.exists(
        gt_json
    ), "gt_json {} does not exist".format(gt_json)  # noqa
    with open(gt_json) as f:
        json_info = json.load(f)
    files = get_pascal_panoptic_files(image_dir, gt_dir, json_info)
    ret = []
    for image_file, label_file, segments_info in files:
        segments_info = [_convert_category_id(x, meta) for x in segments_info]
        part_label_file = (
            image_file.replace("images", "parts_107").replace(".jpg", ".png")
        )
        sem_label_file = (
            image_file.replace("images", "semantic").replace(".jpg", ".png")
        )
        ret.append(
            {
    ...
    ), "sem_seg_file_name {} is not a file".format(ret[0]["sem_seg_file_name"])  # noqa
    assert PathManager.isfile(
        ret[0]["pan_seg_file_name"]
    ), "pan_seg_file_name {} is not a file".format(ret[0]["pan_seg_file_name"])  # noqa
    assert PathManager.isfile(
        ret[0]["part_seg_file_name"]
    ), "part_seg_file_name {} is not a file".format(ret[0]["part_seg_file_name"])  # noqa
    return ret


_RAW_PASCAL_PANOPTIC_SPLITS = {
    "pascal_panoptic_parts_107_train": (
        "pascal/images/training",
        "pascal/panoptic/training",
        "pascal/panoptic/panoptic_training.json",
    ),
    "pascal_panoptic_parts_107_val": (
        "pascal/images/validation",
        "pascal/panoptic/validation",
        "pascal/panoptic/panoptic_validation.json",
    ),
}
...
def register_all_pascal_panoptic_parts_107(cfg):
    ...
    sid_things = eval_spec.eval_sid_things
    sid_stuff = eval_spec.eval_sid_stuff
    sid_total = eval_spec.eval_sid_total

    thing_classes = [eval_spec.eval_sid2scene_label[sid] for sid in sid_things]
    thing_colors = [eval_spec.dataset_spec.scene_color_from_sid(sid) for sid in sid_things]
    stuff_classes = [eval_spec.eval_sid2scene_label[sid] for sid in sid_stuff]
    stuff_colors = [eval_spec.dataset_spec.scene_color_from_sid(sid) for sid in sid_stuff]
    ...
    for i, sid in enumerate(sid_total):
        if sid in sid_things:
            thing_dataset_id_to_contiguous_id[sid] = i
        else:
            stuff_dataset_id_to_contiguous_id[sid] = i
    ...
    for key, (image_dir, gt_dir, gt_json) in _RAW_PASCAL_PANOPTIC_SPLITS.items():
        image_dir = os.path.join(root, image_dir)
        gt_dir = os.path.join(root, gt_dir)
        gt_json = os.path.join(root, gt_json)

        DatasetCatalog.register(
            key, lambda x=image_dir, y=gt_dir, z=gt_json: load_pascal_panoptic_parts(x, y, z, meta)
        )
        ...
        MetadataCatalog.get(key).set(
            ...
            gt_dir=gt_dir.replace("pascal_panoptic_", ""),
            evaluator_type="pascal_panoptic_seg",
            ignore_label=255,
            label_divisor=1000,
            **meta,
        )

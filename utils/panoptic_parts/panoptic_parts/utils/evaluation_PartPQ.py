...
def get_traceback(f):
  ...
  def wrapper(*args, **kwargs):
    try:
      ...
      print('Caught exception in worker thread:', flush=True)
      traceback.print_exc()
      raise e
...
@get_traceback
def evaluate_single_core(proc_id, fn_pairs, pred_reader_fn, spec, cpu_affinity=None):
  ...
  print("core {}: {}/{}".format(proc_id, counter, len(fn_pairs)), flush=True)
  # Loop over all predictions
  for fn_pair in fn_pairs:
    counter += 1
    if counter % 5 == 0: # print progress each 5 steps
      # print(f'core {proc_id}: {counter}/{len(fn_pairs)}')
      print("core {}: {}/{}".format(proc_id, counter, len(fn_pairs)), flush=True)
    ...
    try:
      ...
      print("error reading file: ", gt_pan_part_file, flush=True)
...
def evaluate_PartPQ_multicore(spec,
  ...
  evaluate_single_core_fn = partial(evaluate_single_core, cpu_affinity=cpu_aff)

  fn_splits = np.array_split(filepaths_pairs, cpu_num)
  print("Number of cores to be used: {}, images per core: {}".format(cpu_num, len(fn_splits[0])), flush=True)
  workers = multiprocessing.Pool(processes=cpu_num)
  processes = []
  for proc_id, fn_split in enumerate(fn_splits):
      p = workers.apply_async(evaluate_single_core_fn, (proc_id, fn_split, pred_reader_fn, spec))
      processes.append(p)
